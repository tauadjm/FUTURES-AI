...
- `距EMA幅度%` 分级解读（正值=价格在EMA上方，负值=在EMA下方；以绝对值分级）：
  - ≤0.5%：EMA区域内，入场条件最优（即`价格在EMA附近`=true）
  - 0.5%~1.5%：轻度偏离，结合强信号棒和清晰结构仍可考虑入场
  - 1.5%~3%：中度偏离，入场门槛显著提升，需极强信号棒+明确的二次入场理由
  - >3%：严重偏离，风险大幅升高，通常应等待更深回调至EMA附近再考虑入场
...
- **突破确认（H1~H4通用）**：信号棒收盘价**突破`回调段最高价`**，视为一个独立入场理由，适用于任意H形态（H1/H2/H3/H4）、任意回调根数
...
- 首次EMA缺口棒（`首次EMA缺口确认`=true）→ 第一根重新进入EMA缺口区域的K线；原趋势经历深回调后恢复的强信号，须配合H2/L2确认；可靠性中等
...
- **突破确认（L1~L4通用）**：信号棒收盘价**突破`回调段最低价`**，视为一个独立入场理由，适用于任意L形态（L1/L2/L3/L4）、任意反弹根数
...
- 须结合反转两要素（极值测试+二次确认）才能入场做反转，单独不构成入场理由
...
- 有持仓 + 出现反向强信号 或 已到止盈区域 → "平仓"
  ▌平仓标准（两要素缺一不可，否则选持有）：
    1. 价格测试趋势极值（不足测试=更低高点/更高低点 或 过冲测试=更高高点/更低低点）
    2. 出现二次反转信号（首次反转信号常失败，等待第二次确认）
- 有持仓 + 方向完全反转且需立即反手
"""
...
- `ema_dist_pct` interpretation (positive = price above EMA, negative = below; use absolute value):
  - ≤0.5%: within EMA zone — optimal entry conditions (`near_ema`=true)
  - 0.5%–1.5%: mildly extended — still acceptable with a strong signal bar and clear structure
  - 1.5%–3%: moderately extended — entry threshold significantly higher; requires very strong bar + a second independent reason
  - >3%: severely extended — risk rises sharply; generally wait for a deeper pullback toward EMA before considering entry
...
- H1 (extreme trend): pullback ends after only one leg (`pullback_label`="H1")
  ...
  ⚠️ Pullback bar count filter (`pullback_bars`):
    ...
    - ≥ 5 bars: deep pullback, exemption fully revoked — must have **2 independent** entry reasons to enter; otherwise output wait
...
- **Breakout confirmation (universal for H1–H4)**: signal bar **close breaks above `pullback_high`** counts as one independent entry reason, applicable to any H pattern (H1/H2/H3/H4) at any pullback bar count
...
- First EMA gap bar (`first_ema_gap`=true) → first bar re-entering the EMA gap zone after a deep pullback; strong trend resumption signal, confirm with H2/L2; medium reliability
...
- L1 (extreme trend): bounce ends after only one leg (`pullback_label`="L1")
  ...
  ⚠️ Bounce bar count filter (`pullback_bars`):
    ...
    - ≥ 5 bars: deep bounce, exemption fully revoked — must have **2 independent** entry reasons to enter; otherwise output wait
...
- **Breakout confirmation (universal for L1–L4)**: signal bar **close breaks below `pullback_low`** counts as one independent entry reason, applicable to any L pattern (L1/L2/L3/L4) at any bounce bar count
...
- Must combine with two reversal conditions (extreme test + secondary confirmation) before entering counter-trend; not sufficient alone
...
- In position + strong counter signal appears OR price has reached TP zone → "close"
  ▌Close criteria (BOTH required, otherwise hold):
    1. Price tests trend extreme (undershoot = lower high/higher low; or overshoot = higher high/lower low)
    2. Secondary reversal signal confirmed (first reversal signal often fails; wait for second confirmation)
