...
**第一代（单段 Prompt）**：一个超长中文 Prompt 包含全部规则，有仓/无仓复用同一份。实盘中发现持仓管理和入场规则互相干扰，且超长上下文推理质量不稳定。
...
**代码层前置过滤**：实盘中发现部分情形（首尾盘、震荡区间）即使进入 AI 分析也会浪费 token，因为答案是确定的。迭代后将这些"无需看图即可判断"的二元否决逻辑完全移出 Prompt，改在代码层强制执行：震荡区间直接跳过 AI 调用；无持仓时首尾盘跳过；有持仓时尾盘调用 AI 后强制覆盖为平仓。Prompt 不再描述这些规则，既减少无效请求，也让 Prompt 专注于真正需要模型判断的形态分析。
